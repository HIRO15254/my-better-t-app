# my-better-t-app — Project Guide for Claude

> **テンプレート初期値**: このファイルはテンプレート由来の初期値です。features 名や参照テストパスなどプロジェクト固有の事実は、最初の機能着手時に上書きしてください。
...
- **Server**: Hono on Bun, tRPC v11 server, Better Auth.
- **DB**: PostgreSQL via Drizzle ORM. Migrations in `packages/db/src/migrations`.
...
Pre-PR verification: `bun run check && bun run check-types && bun run test`.
...
`apps/web/src/` layout (template default — adjust to your domain):
...
shared/
  ...
  components/              cross-feature composites (page-header, sign-in-form, ...)
...
- **UI copy is English-only** (template default). No Japanese in user-facing strings (labels, empty states, toasts, errors). Japanese is fine in comments, commit messages, and PR descriptions.
- **Mobile forms are bottom sheets.** Use shadcn `Drawer`, not `Dialog`.
- **Pages start with `PageHeader`** (`apps/web/src/shared/components/page-header/`). Do not hand-roll titles / action rows.
...
Every code change must be test-driven.
...
**Reusable test helpers (template-provided)**:

| Target | Project | Helper file |
|---|---|---|
| Web — QueryClient/Provider/tRPC/auth/toast mocks | `web-dom` | [`apps/web/src/__tests__/test-utils.tsx`](apps/web/src/__tests__/test-utils.tsx) |
| API router (Zod + procedure enumeration) | `api` | [`packages/api/src/__tests__/test-utils.ts`](packages/api/src/__tests__/test-utils.ts) |

> Replace this section with your team's reference patterns once you have a comprehensive coverage sweep. Match the rigor: full branch coverage, error paths, boundary values.
...
If a target does not match any helper above, extend the relevant `test-utils` file with a new helper rather than hand-rolling a new pattern per test file.

### Do NOT run the full test suite during a task

`bun run test` boots jsdom/node for every workspace and takes several minutes. While iterating, run only what's relevant:
